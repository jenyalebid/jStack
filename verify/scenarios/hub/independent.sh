...
rel="$(sed -n 's/.*"\(build\|release\)": "\([^"]*\)".*/\2/p' "$(p=/Applications/jStack\ Hub.app/Contents/Resources/packages; [ -f "$p/build-identity.json" ] && echo "$p/build-identity.json" || echo "$p/release-identity.json")" 2>/dev/null)"
