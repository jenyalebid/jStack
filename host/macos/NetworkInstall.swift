...
// Written by no installer any more: the root watchdog it armed is deleted.
// Kept so that cutover and uninstall can clear what an earlier one left.
...
func activate(_ request: InstallRequest, at transaction: URL, work: URL) throws {
    ...
    // Disarm, under the same admin consent that authorized this transaction.
    // The root watchdog this file used to arm existed to rename a retained Hub
    // backup back into place; no update retains one, so it has nothing to
    // restore and is gone. An installer before this one may have left the
    // policy behind, and a policy naming a watchdog is not left lying in
    // /Library/Preferences.
    if manager.fileExists(atPath: hubPolicyPath.path) { try manager.removeItem(at: hubPolicyPath) }
    if policy.active {
        let deadline = Date().addingTimeInterval(30)
        var healthy = false
        while Date() < deadline {
