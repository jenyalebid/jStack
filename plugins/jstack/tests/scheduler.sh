...
p.write_text("\n".join([
    ...
    tool("Grep", pattern="y"),
    tool("Bash", command="git log --oneline -5 | head -3"),
...
check "Bash is read by its command line, not by its name" '
...
check "a missing transcript reads as no action; one that cannot be read blocks" '
from pathlib import Path
from scheduler import config, runner
# A run killed on TTFT never reached the model — the CLI writes the opening turn
# as the session starts, so no file means nothing happened. That case IS the
# retry arm, and must keep retrying.
assert runner.first_action(Path(config.STATE_DIR) / "never-written.jsonl") is None
